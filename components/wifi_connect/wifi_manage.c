...
void handle_wifi_event(void *event_handler_arg, esp_event_base_t event_base, int32_t event_id, void *event_data){
    uint8_t* ssidH = (uint8_t*)event_handler_arg;
    ...
    if(event_id == WIFI_EVENT_STA_CONNECTED){
        ESP_LOGI(TAG_Wifi,"WiFi Connect to network %s",ssidH);
        xEventGroupSetBits(WiFi_Event_Group, WIFI_CONNECT_BIT);
    }
    if(event_id == WIFI_EVENT_STA_DISCONNECTED){
        if(Retry_Connect < 1){
            ESP_LOGI(TAG_Wifi,"WiFi Disconnected from network %s",ssidH);
        }
...
void WiFi_Reconnect_Task(void* const pvParameter){
    uint8_t* ssidH = (uint8_t*)pvParameter;
    Retry_Connect = 0;

    while(1){
        EventBits_t bit = xEventGroupWaitBits(
            WiFi_Event_Group,
            WIFI_DISCONNECT_BIT | WIFI_CONNECT_BIT,
            pdTRUE,
            pdFALSE,
            portMAX_DELAY
        );
        if(bit & WIFI_DISCONNECT_BIT){
            if(Retry_Connect < RETRY_CONNECT_LIMIT){
                Retry_Connect++;
                ESP_LOGI(TAG_Wifi,"Reconnecting to the network %s (attempt %d)", ssidH, Retry_Connect );
                esp_wifi_connect();
...
esp_netif_init();
    ...
    esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, handle_wifi_event,(void*)WiFi_cfg.sta.ssid);

    WiFi_Event_Group = xEventGroupCreate();
    TaskHandle_t Task_handle;
    xTaskCreate(WiFi_Reconnect_Task, "WiFi_Reconnect_Task", 4096, (void*)WiFi_cfg.sta.ssid, 1, &Task_handle);
